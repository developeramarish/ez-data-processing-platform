...
const FormulaBuilder: React.FC<FormulaBuilderProps> = ({
  ...
  // Initialize from props when they change (e.g., when loading existing metric)
  useEffect(() => {
    if (formula && formula !== customFormula) {
      setCustomFormula(formula);
      setExpressionValid(validatePromQLExpression(formula));
    }
    if (formulaType && formulaType !== selectedType) {
      setSelectedType(formulaType);
    }
  }, [formula, formulaType, customFormula, selectedType]);
  ...
  // Update formula when type or template changes
  useEffect(() => {
    ...
    } else if (selectedType === 'promql') {
      if (selectedTemplate !== 'custom') {
        const newFormula = generateFormula(selectedTemplate, fieldPath);
        setCustomFormula(newFormula);
        onChange(newFormula, 'promql');
        setExpressionValid(validatePromQLExpression(newFormula));
      } else {
        // Custom expression - validate existing
        setExpressionValid(validatePromQLExpression(customFormula));
      }
    } else if (selectedType === 'recording') {
      // Recording rule reference
      onChange(customFormula, 'recording');
      setExpressionValid(customFormula.length > 0);
    }
  }, [selectedType, selectedTemplate, fieldPath]);
