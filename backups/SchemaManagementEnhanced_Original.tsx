...
import { 
  ...
  usePublishSchema,
  useDuplicateSchema 
} from '../../hooks/useSchemaApi';
import { SchemaListParams, CreateSchemaRequest, SchemaStatus } from '../../types/schema-api';
import {
  PlusOutlined,
  SearchOutlined,
  EditOutlined,
...
const SchemaManagementEnhanced: React.FC = () => {
  ...
  // API hooks
  const schemaParams: SchemaListParams = {
  ...
  // Helper function to calculate field count from JSON schema
  ...
  // Transform API data to local format - backend uses PascalCase - memoize to prevent useEffect issues
  const schemas: SchemaRecord[] = React.useMemo(() => schemasResponse?.data?.map((schema: any) => ({
    id: schema.ID,
    name: schema.Name,
  ...
  const loading = isLoading || createSchemaMutation.isPending;

  // Fetch data sources for dropdown - define outside useEffect so it can be called after operations
  ...
  const handleDuplicate = async (record: SchemaRecord) => {
    try {
      await duplicateSchemaMutation.mutateAsync({
        id: record.id,
        request: {
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteSchemaMutation.mutateAsync({ id, deletedBy: 'User' });
    } catch (error) {
  ...
  const handleSubmitCreate = async (values: any) => {
    try {
      // Create a valid blank JSON Schema structure
      ...
      const newSchema = await createSchemaMutation.mutateAsync(payload);
  ...
  const handlePublishSchema = async (id: string) => {
    try {
      await publishSchemaMutation.mutateAsync(id);
    } catch (error) {
  ...
  const handleStatusChange = async (id: string, newStatus: 'Draft' | 'Active' | 'Inactive' | 'Archived') => {
    try {
      // First get the current schema data from API
      ...
      // Convert string status to enum number for backend
      ...
      // IMPORTANT: Backend returns PascalCase properties
      // Use fetch directly to send number instead of string
      ...
      // Refetch to revert the optimistic UI update
      await refetch();
    }
  };
  ...
  const handleDataSourceAssignment = async (schemaId: string, dataSourceId?: string) => {
    try {
      // CRITICAL FIX: Fetch FRESH schema data from API before checking
      // Don't rely on stale React state
      console.log('Fetching fresh schema data from API...');
      ...
      if (dataSourceId) {
        // STRICT 1-to-1 ENFORCEMENT: Block assignment if data source already assigned
        console.log('Checking for existing assignment of', dataSourceId);
        console.log('Fresh schemas from API:', freshSchemas.map((s: any) => ({ 
          id: s.ID, 
          name: s.DisplayName, 
          dsId: s.DataSourceId 
        })));
        ...
        if (existingAssignment) {
          console.log('BLOCKING: Data source already assigned to', existingAssignment.DisplayName);
          Modal.error({
          ...
          // Revert the UI by refetching to show the original value
          await refetch();
          return;
        }
      }
  ...
  const performAssignment = async (schemaId: string, dataSourceId: string | undefined, schemaDisplayName: string) => {
    console.log('performAssignment called:', { schemaId, dataSourceId, schemaDisplayName });
    ...
    try {
      ...
      console.log('Current schema fetched:', getData);
      ...
      // Prepare update payload - if dataSourceId is undefined or empty, send null
      const payload: any = {
        displayName: currentSchema.DisplayName,
        description: currentSchema.Description,
        jsonSchemaContent: currentSchema.JsonSchemaContent,
      ...
      // Only include dataSourceId if it has a value, otherwise send null
      if (dataSourceId) {
      ...
      // Refresh both schemas and data sources to keep dropdowns in sync
      console.log('Calling refetch...');
      await refetch();
      await fetchDataSources(); // Refresh data sources dropdown after assignment changes
      console.log('Refetch complete');

      // Show verification success message
      if (dataSourceId) {
        console.log('Showing assignment success message');
        message.success(`✓ ${schemaDisplayName} קושר למקור נתונים - עודכן בהצלחה`);
      } else {
        console.log('Showing unassignment success message');
  ...
  // Handle templates - show available schema templates
      ...
      content: (
        <List
          ...
          renderItem={(template: any) => (
            ...
            >
              <List.Item.Meta
                title={<Text strong>{template.nameHebrew}</Text>}
                description={template.descriptionHebrew}
              />
            </List.Item>
          )}
        />
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (dataSourceId: string, record: SchemaRecord) => {
        // Filter data sources to show only:
        // 1. The currently assigned data source (if any)
        // 2. Data sources not assigned to ANY other schema
          ...
          <Form.Item style={{ marginBottom: 0 }}>
            <Space style={{ width: '100%', justifyContent: 'flex-end' }}>
              ...
              <Button 
                type="primary" 
                htmlType="submit" 
                loading={createSchemaMutation.isPending}
              >
                צור
              </Button>
            </Space>
