...
const SchemaFieldSelector: React.FC<SchemaFieldSelectorProps> = ({
  ...
  const fetchSchema = useCallback(async () => {
    ...
    try {
      ...
      // Backend returns: { isSuccess: true, data: Schema[], total, page, size }
      // So response.data is directly the Schema[] array
      if (response.isSuccess && response.data && Array.isArray(response.data)) {
        console.log('SchemaFieldSelector: Schemas array:', response.data);
        console.log('SchemaFieldSelector: Number of schemas:', response.data.length);

        if (response.data.length > 0) {
          const fetchedSchema = response.data[0];
  ...
  const handleFieldChange = useCallback((value: string) => {
    // Access fields from state ref to avoid dependency
    setFields(currentFields => {
      const field = currentFields.find(f => f.name === value);
      if (field) {
        onFieldSelect(value, field);
      }
      return currentFields; // Return unchanged to avoid state update
    });
  }, [onFieldSelect]);

  const handleFilterOption = useCallback((input: string, option: any) => {
  ...
  }, [dataSourceId]); // fetchSchema/fetchSchemaMetrics are memoized with useCallback(dataSourceId) - only need dataSourceId here
