...
interface AlertRuleBuilderProps {
  ...
  currentMetricName?: string; // The metric being edited - auto-select for alerts
}
...
const AlertRuleBuilder: React.FC<AlertRuleBuilderProps> = ({
  ...
  }, [rules]); // Only depend on rules, not onChange to prevent infinite loop
  ...
  const handleTemplateSelect = (templateId: string) => {
    ...
    if (template) {
      ...
      template.parameters.forEach(param => {
        ...
        // Auto-fill metric parameter with current metric being edited
        if (param.type === 'metric' && currentMetricName) {
          initParams[param.name] = currentMetricName;
        }
      });
