...
const VisualFormulaBuilder: React.FC<VisualFormulaBuilderProps> = ({
  ...
  const generateFormula = React.useCallback(() => {
    if (!selectedField) {
      setGeneratedFormula('');
      return;
    }
  ...
  }, [selectedField, selectedFunction, whereCondition, groupByEnabled, groupByField, timeWindow, customWindow, onFormulaGenerated]);

  useEffect(() => {
    // TODO: Fetch actual fields from schema API based on dataSourceId
    setAvailableFields(mockFields);
  }, [dataSourceId, mockFields]);
